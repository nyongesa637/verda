...
export default async function ForbiddenPage() {
  ...
  // Localised body wraps the role token in <code> at the {role} boundary so
  // the styling survives even when surrounding words reorder for the locale.
  const body = t("forbidden.body", { role: "lawyer" });
  const segments = body.split(/(lawyer)/);
      ...
      <div className="grid w-full gap-6 sm:grid-cols-[auto_1fr] sm:items-center sm:gap-8">
        ...
        <div>
          ...
          <p className="mt-3 max-w-prose text-sm text-ink/65 leading-relaxed pretty">
            {segments.map((s, i) =>
              s === "lawyer" ? (
                <code key={i} className="mono text-[12px]"> lawyer</code>
              ) : (
                <span key={i}>{s}</span>
              ),
            )}
